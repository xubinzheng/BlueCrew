...
/**---------------------------------------------------------------------------------------
 * @name Fetching, Saving, Refreshing, and Deleting an Object
 * ---------------------------------------------------------------------------------------
 */

/** Fetch the object with the specified objectId from the Mobile Data service.

 The fetchObjectWithId method is a provisional API.  APIs that are marked provisional are evolving and might change or be removed in future releases.

 If successful, the result value of the returned BFTask contains a reference to the fetched object.

 @param objectId The objectId for the object to be returned
 @return the BFTask that can be used to receive the result when the fetch completes.
 */
+ (BFTask*)fetchObjectWithId:(NSString *)objectId;
